  ...
  def main(args: Array[String]){
    ...
    for(iteration <- 0 until maxiter){
      for(subepoch <- 0 until executor_num){
        ...
        data_array_with_w += cjoined4es(idx).mapPartitions(x=> wrapper(x), true).persist(StorageLevel.MEMORY_ONLY)
        h_array += cjoined4es(idx).map(x => update_h_ids_onlymap(x, executor_num)).persist(StorageLevel.MEMORY_ONLY)
      }
      ...
      for(subepoch <- 0 until executor_num){
        val idx = iteration*executor_num + subepoch
        eval_h_array += h_array(iteration*executor_num + executor_num).map(x => update_eval_h_ids_onlymap(x, subepoch, executor_num)).persist(StorageLevel.MEMORY_ONLY)
