...
# to count the total number of bytes read/written via file system using strace
# $ strace -e trace=read,write -o strace.log ./ooc_sgd_mf.py RCV1.small_train.csv 16 2 tmp
# $ awk '{s+=$NF} END {print s}' strace.log
...
if __name__ == "__main__":
    ...
    for i in range(0, num_iterations):
        ...
        error, n = evaluate_block_by_block(N, x_block_dim,
                                           y_block_dim)
        t2 = time.clock()
        print i, int(t2 - t1), error, math.sqrt(error / n)

    print "Stochastic Gradient Descent Done, total time =", int(t2 - t1), "seconds, exit now"
